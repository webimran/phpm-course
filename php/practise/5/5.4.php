<?php
    ...
    if('encode' == $task){
        $data = $_POST['data'] ?? '';
        if($data != ''){
            $data = scrambledData($data, $key);
        }
    }
    if('decode' == $task){
        $data = $_POST['data'] ?? '';
        if($data != ''){
            $data = decodeData($data, $key);
        }
    }
?>
        ...
        <div class="container">
            <div class="row">
                <div class="column column-60 column-offset-20">
                    ...
                    <p>
                        <a href="/5.4.php?task=encode">Encode</a> | 
                        <a href="/5.4.php?task=decode">Decode</a> |
                        <a href="/5.4.php?task=key">Generate Key</a>
                    </p>
                </div>
            </div>
            <div class="row">
                <div class="column column-60 column-offset-20">
                    <form action="5.4.php<?php if('decode' == $task){ echo '?task=decode'; }?>" method="POST">
